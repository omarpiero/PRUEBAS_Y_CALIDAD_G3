# 16. Repositorio oficial y modulos por integrante

Fecha de cierre: 2026-06-27  
Repositorio oficial definido para el informe: `https://github.com/omarpiero/PRUEBAS_Y_CALIDAD_G3.git`  
Rama local de trabajo: `main`

## Decision de unificacion

Para evitar contradicciones en la sustentacion, el informe final debe citar una sola fuente Git. Se adopta como repositorio oficial el enlace entregado por el jefe de grupo:

```text
https://github.com/omarpiero/PRUEBAS_Y_CALIDAD_G3.git
```

Toda referencia antigua a `ROGERCanchumanyaUC/pruebas-calidad-grupo-03` debe tratarse como historica o reemplazarse por el enlace oficial anterior.

## Tabla para el informe final

| Integrante | Rol sugerido de sustentacion | Modulo/evidencia a declarar | Archivos o evidencias base | Estado |
| --- | --- | --- | --- | --- |
| Omar Piero Terbullino Jaime | Coordinacion, Git, arquitectura y despliegue | Repositorio, integracion general, despliegue/demo y cierre documental | `git log`, `documentacion/cierre_informe_final/`, `CHECKLIST_DEPLOY_LMS.md` | Confirmar con commits reales |
| Giancarlo Guerrero Cordova | QA, pruebas e ISO | Plan de pruebas, ISO 29119, metricas, evidencia de ejecucion | `tests/`, `documentacion/PRUEBAS/`, `documentacion/ISOS/` | Confirmar con commits reales |
| Roger Moises Canchumanya Avellaneda | Diseno, matriz, macroprocesos y legal | Matriz doble entrada, macroprocesos, capturas/mockups, INDECOPI | `documentacion/PENDIENTES/`, `documentacion/CAPTURAS/`, `documentacion/INDECOPI/` | Confirmar con commits reales |

## Evidencia minima por integrante

Cada integrante debe presentar:

1. Modulo o paquete de trabajo.
2. Archivos modificados.
3. Hashes de commit.
4. Captura o enlace de GitHub.
5. Breve explicacion de integracion al producto final.

## Texto sugerido para el Word

El codigo fuente del proyecto se centraliza en GitHub como fuente de verdad tecnica. Para la entrega final se utiliza el repositorio `https://github.com/omarpiero/PRUEBAS_Y_CALIDAD_G3.git`, rama `main`. Cada integrante sustenta el modulo asignado y su integracion al repositorio mediante evidencia de commits, archivos modificados y relacion con los requerimientos del LMS.

## Criterio de aceptacion

Este punto queda cerrado cuando:

- El Word final cita un solo repositorio.
- La tabla de integrantes incluye modulo y commits.
- La presentacion usa el mismo repositorio.
- El push final queda visible en GitHub o se adjunta evidencia del bloqueo de permisos.
